...
void LCD_Draw_FaceBox(void)
{
	...
	struct FACE faceInfo;
	for (uint8_t faceNum = 0; faceNum < num; faceNum++)
	{
		faceInfo = ACC_ReadResults(faceNum);
		if(faceInfo.mask==0){
			LCD_Thick_Window(faceInfo.coord[0],faceInfo.coord[1],faceInfo.coord[2],faceInfo.coord[3],GREEN);
		}
		else if (faceInfo.mask==1)
		{
			LCD_Thick_Window(faceInfo.coord[0],faceInfo.coord[1],faceInfo.coord[2],faceInfo.coord[3],RED);
		}
	}
...
void LCD_DMA(uint32_t SrcAddr, uint8_t Size, uint32_t Len)
{
	LCD_PrepareDMA();
    DMA(SrcAddr,0x6009105C,(Size|0x04),Len);
    LCD_QuitDMA();
}

// void LCD_Display(uint8_t num)
// {
// 	LCD_DMA((&IMAGE->IMAGESRC[num][0]),2,76800);
// }

void LCD_Display(void)
{
    LCD_DMA((&CAMERA->CAMERA_VALUE[0][0]),2,76800);
}
